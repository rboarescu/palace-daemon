...
- **System Service Only:** The live service is `mempalace-hub` (`sudo systemctl [start|stop|restart] mempalace-hub`); palace-daemon is retired.
...
### 5. Status (2026-09-05)
This project is ARCHIVED. The palace is served by upstream `mempalace serve` as `mempalace-hub.service`.
Upgrade path is now: `mempalace update plan` → `pipx upgrade mempalace` → `sudo systemctl restart mempalace-hub`.
No local patches exist.
